...
export default function School() {
  ...
  const subjects = [...new Set(schoolAssignments.map(a => a.subject))];
  const filteredAssignments = schoolAssignments.filter(a => filterSubject === 'all' || a.subject === filterSubject);
  ...
  const handleSave = () => {
    ...
    if (isEditing) {
      dispatch({
        type: 'UPDATE_SCHOOL_ASSIGNMENT',
        payload: { id: currentAssignment.id, updates: currentAssignment }
      });
    } else {
      dispatch({
        type: 'ADD_SCHOOL_ASSIGNMENT',
  ...
  const toggleComplete = (assignment) => {
    dispatch({
      type: 'UPDATE_SCHOOL_ASSIGNMENT',
      payload: { id: assignment.id, updates: { completed: !assignment.completed } }
    });
  };
              ...
              <div className="space-y-4">
                {filteredAssignments.length === 0 ? (
                  <p className="text-center text-gray-500">
                    ...
                    <span className="underline cursor-pointer" onClick={openNewDialog}> Add one?</span>
                  </p>
                ) : (
                  filteredAssignments
                    .sort((a,b) => new Date(a.due_date) - new Date(b.due_date))
                    .map(a => (
                      <div 
                        ...
                        className={`flex items-start gap-4 p-4 rounded-lg ${
                          a.completed ? 'bg-green-50' : 'bg-white'
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className={`font-semibold ${a.completed ? 'line-through text-gray-500' : ''}`}>
                            {a.title}
                          </p>
                          <div className="flex items-center gap-4 text-sm text-gray-500 mt-1">
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              Due: {format(parseISO(a.due_date), 'MMM dd, yyyy')}
                            </span>
                          </div>

...
export default function Progress() {
  const { state } = useApp();
  const { userHabits, habits, habitLogs, user } = state;
  
  const [selectedPeriod, setSelectedPeriod] = useState("7days");
  const [weeklyData, setWeeklyData] = useState([]);

  useEffect(() => {
    if (userHabits.length > 0 && habitLogs.length > 0) {
      generateWeeklyData();
    }
  }, [habitLogs, userHabits, selectedPeriod]);
          ...
          <CardContent>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {userHabits.map((userHabit) => {
                ...
                const habitLogs = habitLogs.filter(log => log.user_habit_id === userHabit.id);
                const completedLogs = habitLogs.filter(log => log.completed).length;
